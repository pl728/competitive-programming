...
//        }
        ...
        for (int length = 1; length <= n; length++) {
            // for each length of [L,R] we have (n - len + 1)! * (len)! permutations
            // for each length [l, r] we have len pairs
            long temp = ((factorials[n - length + 1] % m)  * (factorials[length] * length % m)) % m;
            ans = ((ans % m) + (temp % m)) % m;
        }
