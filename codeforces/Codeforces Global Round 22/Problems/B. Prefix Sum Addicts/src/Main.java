...
public class Main {
    ...
    public static void main(String[] args) {
        ...
        int tc = sc.ni();
        while (tc-- != 0) {
            int n = sc.ni(); int k = sc.ni();
            int[] s = sc.readIntArray(k);

            // generating the last k elements of a, and making sure it is sorted,
            if(k == 1) {
            ...
            boolean flag = true;
            for(int i = k-1; i >= 1; i--) {
                if(s[i] - s[i-1] > difference) {
                    flag = false;
                    break;
                }
                difference = s[i] - s[i-1];
            }
            // can we have the first part of a consist of numbers <= than difference and add up to s[0]?

            if(!flag) {
                // prefix sums has error
                System.out.println("No");
                continue;
            }

            if((n - k + 1) * difference < s[0]) {
                flag = false;
            }
            ...
            // after generating the last k elements of a, and making sure it is sorted,
            // check whether it's possible to have a non-decreasing seq of numbers for the first part of a
        }
    }
