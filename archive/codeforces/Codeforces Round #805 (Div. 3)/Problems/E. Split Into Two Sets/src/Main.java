...
public class Main {
    ...
    public static void main(String[] args) {
        ...
        // if undirected graph has cycle of length 3,
        int t = sc.nextInt();
        while(t-- != 0) {
            int n = sc.nextInt();
            Set<Integer> pile1 = new HashSet<>();
            Set<Integer> pile2 = new HashSet<>();
            boolean ans = true;
            for(int i = 0; i < n; i++) {
                int a = sc.nextInt();
                int b = sc.nextInt();
                ...
                if(!pile1.contains(a) && !pile1.contains(b)) {
                    pile1.add(a);
                    pile1.add(b);
                } else {
                    if(!pile2.contains(a) && !pile2.contains(b)) {
                        pile2.add(a);
                        pile2.add(b);
                    } else {
                        ans = false;
