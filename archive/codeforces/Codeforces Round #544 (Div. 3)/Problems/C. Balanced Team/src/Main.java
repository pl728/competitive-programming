...
public class Main {
    ...
    public static void main(String[] args) {
        ...
        int n = sc.nextInt();
        Long[] a = sc.readLongArrayObject(n);

        // 1 10 17 12 15 2
        // 1 2 10 12 15 17

        Arrays.sort(a);     // only passed TLE when we used Long[] instead of long[]
        int ans = 0;
        int i = 0;
        int j = 1;
        while (j < n) {
    ...
    static class FastReader {
        ...
        public Long[] readLongArrayObject(int n) {
            Long[] result = new Long[n];
            for (int i = 0; i < n; i++) {
                result[i] = this.nextLong();
            }
