...
public class Main {
    ...
    public static void main(String[] args) {
        ...
        while (tc-- != 0) {
            ...
            // current sum cannot be <= 0 before we reach the last nonzero element
            int lastNonZeroElem = n - 1;
            while(lastNonZeroElem != -1 && a[lastNonZeroElem] == 0) lastNonZeroElem--;
            if(lastNonZeroElem == -1) {
                System.out.println("Yes");
                continue;
            }
            long currSum = a[0];
            ...
            boolean ans = true;
            for(int i = 1; i <= lastNonZeroElem; i++) {
                currSum += a[i];
                if(currSum <= 0 && i != lastNonZeroElem) {
                    ans = false;
                    break;
                }
            }
