...
public class Main {
    ...
    public static void main(String[] args) {
        ...
        int t = sc.nextInt();
        while(t-- != 0) {
            // if there is a number that is smaller than any two numbers on the left or right to it, output no
            int n = sc.nextInt();
            long[] a = sc.readLongArray(n);
            long decreasing_threshold = Long.MAX_VALUE;
            boolean ans = true;
            for(int i = 1; i < n; i++) {
                if(a[i] > decreasing_threshold) {
                    ans = false;
                    break;
                }
                if(a[i-1] > a[i]) {
                    decreasing_threshold = Math.min(decreasing_threshold, a[i]);
                }
            }
            if(ans) {
