...
public class Main {
    ...
    public static void main(String[] args) {
        ...
        while (tc-- != 0) {
            ...
            Map<Character, Character> cycleMap = new HashMap<>();
            for(int i = 1; i <= 26; i++) {
                cycleMap.put(i2c(i), i2c(i));
            }
            Map<Character, Character> mapping = new HashMap<>();

            // for each character, find the lowest mapping such that there is no closed loop

            for(int i = 0; i < n; i++) {
                boolean flag = true;
                for(int j = 1; j <= 26 && flag; j++) {
                    if(cycleMap.containsKey(s.charAt(i))) {
                        if(cycleMap.size() == 2) {
                            System.out.println(cycleMap);
                            cycleMap.put(c, cycleMap.get(s.charAt(i)));
                            cycleMap.remove(s.charAt(i));
                            break;
                        }
                        if(cycleMap.get(s.charAt(i)) == i2c(j)) {
                            continue;
                        }

                        mapping.put(s.charAt(i), i2c(j));
                        for(char c : cycleMap.keySet()) {
                            if(cycleMap.get(c) == i2c(j)) {
                                cycleMap.put(c, cycleMap.get(s.charAt(i)));
                                cycleMap.remove(s.charAt(i));
                                flag = false;
                                break;
            ...
            for(int i = 0; i < n; i++) {
                sb.append(mapping.get(s.charAt(i)));
            }
