...
public class Main {
    ...
    public static void main(String[] args) {
        ...
        // count number of each letter to remove
        // iterate through the string to reconstruct the new string
        int[] counts = new int[26];
        int[] removed = new int[26];
        for(int i = 0; i < n; i++) {
            counts[lowercaseToIndex(s.charAt(i))]++;
        }
        for(int i = 0; i < 26; i++) {
            if(counts[i] <= k) {
                k -= counts[i];
        ...
        for(int i = 0; i < n; i++) {
            if(removed[lowercaseToIndex(s.charAt(i))] > 0) {
                removed[lowercaseToIndex(s.charAt(i))]--;
            } else {
