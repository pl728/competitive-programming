...
public class Main {
    ...
    public static void main(String[] args) {
        ...
        while (t-- != 0) {
            ...
            // 0?10
            // keep track of # of continuous ?? in current unstable substr.
            // when currChar = prevChar,
            if (n == 1) {
            ...
            if (s.charAt(0) == '?') {
                ...
                prevChar = Integer.parseInt(String.valueOf(s.charAt(0)));
                continuousQ = 0;
            }
            dp[0] = 1;
            ...
            for (int i = 1; i < n; i++) {
                if (s.charAt(i) == '?') {
                    currSubstringLength++;
                    dp[i] = dp[i - 1] + currSubstringLength;
                    if (prevChar == 0) {
                        prevChar = 1;
                    } else {
                        prevChar = 0;
                    ...
                    if (Integer.parseInt(String.valueOf(s.charAt(i))) == prevChar) {
                        // set curr sub length to continuousQ + 1
                        currSubstringLength = continuousQ + 1;
                    } else {
                        currSubstringLength++;
                    }
                    dp[i] = dp[i - 1] + currSubstringLength;
                    prevChar = Integer.parseInt(String.valueOf(s.charAt(i)));
                    continuousQ = 0;
                }
            }
