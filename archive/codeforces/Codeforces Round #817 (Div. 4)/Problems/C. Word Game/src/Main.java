...
public class Main {
    ...
    public static void main(String[] args) {
        ...
        while (tc-- != 0) {
            int n = sc.ni();
            String[] a = sc.readStringArray(n);
            String[] b = sc.readStringArray(n);
            String[] c = sc.readStringArray(n);

            Map<String, Integer> wordCount = new HashMap<>();

            for (int i = 0; i < n; i++) {
                wordCount.putIfAbsent(a[i], 0);
                wordCount.put(a[i], wordCount.get(a[i]) + 1);
                wordCount.putIfAbsent(b[i], 0);
                wordCount.put(b[i], wordCount.get(b[i]) + 1);
                wordCount.putIfAbsent(c[i], 0);
                wordCount.put(c[i], wordCount.get(c[i]) + 1);
            }

            int aScore = 0, bScore = 0, cScore = 0;
            for (int i = 0; i < n; i++) {
                if(wordCount.get(a[i]) == 2) {
                    aScore += 1;
                }
                if(wordCount.get(a[i]) == 1) {
                    aScore += 3;
                }
                if(wordCount.get(b[i]) == 2) {
                    bScore += 1;
                }
                if(wordCount.get(b[i]) == 1) {
                    bScore += 3;
                }
                if(wordCount.get(c[i]) == 2) {
                    cScore += 1;
                }
                if(wordCount.get(c[i]) == 1) {
                    cScore += 3;
                }
            }
