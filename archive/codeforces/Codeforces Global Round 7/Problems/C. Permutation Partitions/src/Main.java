...
public class Main {
    ...
    public static void main(String[] args) {
        ...
        // [1, 2, 3]
        // k = 2
        // sum(3) + sum (3 - 2) = sum(0, n) + sum(0, n - k)
        long a = f(n, k);

        // need indexes of all values between [n-k+1, ... , n]
        ...
        long ans = 1;
        for(int i = 1; i < k; i++) {
            ans = ((ans % modulololol) * ((distances[i] - distances[i - 1]) % modulololol)) % modulololol;
        }
    ...
    static long f(int n, int k) {
        return (((long) n * (n + 1)) / 2) - (((long) (n - k) * ((n - k) + 1)) / 2);
    }
